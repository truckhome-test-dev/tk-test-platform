...
class Bug_Calculate(SqlOperate):
    ...
    #插入质量数据 
	def bugInsert(self,vname,proname,versionname,name,checknum,fristnum,leaknum,newnum,bugcount,bugdensity,fristleak,bringerror,addtime):
		
		self.insData('bugcalculate',{'vname':vname,'proname':proname,'versionname':versionname,'name':name,'checknum':checknum,'fristnum':fristnum,'leaknum':leaknum,'newnum':newnum,'bugcount':bugcount,'bugdensity':bugdensity,'fristleak':fristleak,'bringerror':bringerror,'addtime':addtime})
		data=self.bugnewsel()
		print(data)
		#self.bugdel()
		if data:
	...
	def bugdel(self):
		data =self.buginsert()
		self.dbcur()
		sql = "update bugmidu SET `density`=%.2f, `leakage`=%.2f, `lead`=%.2f,`updatetime`='%s' WHERE addtime='%s'"%(data[0],data[1],data[2], time.strftime('%Y-%m-%d',time.localtime(time.time())), time.strftime('%Y-%m',time.localtime(time.time())))
		self.sqlExe(sql)
		self.sqlCom()
		self.sqlclo()
		return "pass"	
