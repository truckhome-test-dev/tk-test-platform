...
class Monitor_Inform(SqlOperate):
    ...
    def start_inform(self, taskid, apiid, code):
        ...
        if info[5] == 0:
            ...
            self.dbcur()
            sql = "SELECT inform FROM `api_inform` WHERE apiid=%d " % (apiid)
            self.sqlExe(sql)
            data = list(self.cur.fetchall())
            ...
            print ("stopnum:" + str(stopnum) + ",renum:" + str(renum) + ",num:" + str(num))
            ...
            print (ding,content,token,receiver,num)	        
